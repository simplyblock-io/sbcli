...
class SPDKParams(BaseModel):
    ...
    spdk_cpu_mask: Optional[str] = Field(None, pattern=r'^0x[0-9a-zA-Z]+$')
...
def spdk_process_start(body: SPDKParams):
    ...
    total_mem = core_utils.parse_size(body.total_mem) if body.total_mem else ''
    ...
    container = node_docker.containers.run(
        spdk_image,
        f"/root/scripts/run_distr_with_ssd.sh {body.spdk_cpu_mask} {spdk_mem_mib} {spdk_debug}",
...
def get_file_content(path: FilePath):
    out, err, _ = node_utils.run_command(f"cat /etc/simplyblock/{path.file_name}")
    if out:
        return utils.get_response(out)
    elif err:
