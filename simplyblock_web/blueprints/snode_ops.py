...
def delete_gpt_partitions_for_dev(body: utils.DeviceParams):
    pci_utils.ensure_driver(body.device_pci, 'nvme')
    device_name = pci_utils.device_name(body.device_pci)
    subprocess.check_call(f'parted -fs /dev/{device_name} mklabel gpt')
    return utils.get_response(True)
...
def bind_device_to_spdk(body: utils.DeviceParams):
    pci_utils.ensure_driver(body.device_pci, 'uio_pci_generic', set_override=True)
    return utils.get_response(True)
